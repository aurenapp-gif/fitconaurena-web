      ...
      <div className="flex gap-2 mb-6">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setCat(t.key)}
            className={`text-sm font-bold px-4 py-2 rounded-xl transition-colors ${cat === t.key ? "bg-[#1CA0E3] text-white" : "border border-[#252525] text-[#A0A0A0] hover:text-white"}`}>
            {t.label}
          </button>
        ))}
      </div>

      <form onSubmit={publish} className="card-dark p-5 !transform-none border-[#1CA0E3]/30 mb-6">
            ...
            <div key={p.id} className="card-dark p-5 !transform-none">
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  {p.isCoach && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#1CA0E3] text-white shrink-0">COACH</span>
                  )}
                </div>
              ...
              <button
                ...
                className={`mt-3 inline-flex items-center gap-1.5 text-sm transition-colors ${p.liked ? "text-[#1CA0E3]" : "text-[#666666] hover:text-white"}`}
                aria-label="Me gusta"
              >
