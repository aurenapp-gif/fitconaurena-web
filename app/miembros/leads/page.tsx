...
export default async function LeadsPage() {
        ...
        <div className="container-wide relative z-10 py-16">
          ...
          {leads.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              {LEAD_STATUSES.map((s) => (
                <span key={s.value} className="text-xs px-3 py-1.5 rounded-full border border-[#252525] text-[#A0A0A0]">
                  {s.label}: <strong className="text-white">{counts.get(s.value) ?? 0}</strong>
                </span>
              ))}
            </div>
          )}
  ...
  notes text
            ...
            <p className="text-[#A0A0A0]">Aún no hay leads. Aparecerán aquí en cuanto alguien complete el cuestionario de la landing.</p>
          ) : (
